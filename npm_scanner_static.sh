#!/bin/bash
# NPM Compromise Scanner - no time window
# Root / MDM friendly.
#
# Behavior:
...
#       2. Project node_modules under /Users
#       3. Lockfiles under /Users
#
# Output:
...
for U in /Users/*; do
  ...
  [ -d "$U/.node_modules" ] && printf '%s\n' "$U/.node_modules" >> "$GLOBAL_ROOTS_TMP"
  [ -d "$U/.node/lib/node_modules" ] && printf '%s\n' "$U/.node/lib/node_modules" >> "$GLOBAL_ROOTS_TMP"
done
...
done < <(find_project_node_modules)
